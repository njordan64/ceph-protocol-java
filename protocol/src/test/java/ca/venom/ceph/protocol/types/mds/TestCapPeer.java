...
public class TestCapPeer {
    private static final String MESSAGE_PATH = "sample_messages/types/mds/CapPeer/";

    private List<CapPeer> expectedMessages;
    ...
    public void setup() throws Exception {
        ...
        for (int i = 1; i <= 5; i++) {
            String jsonPath = MESSAGE_PATH + "CapPeer_" + i + ".json";
            String binPath = MESSAGE_PATH + "CapPeer_" + i + ".bin";

            InputStream jsonInputStream = TestCapPeer.class.getClassLoader().getResourceAsStream(jsonPath);
            InputStream binInputStream = TestCapPeer.class.getClassLoader().getResourceAsStream(binPath);

            ObjectMapper objectMapper = new ObjectMapper();
            CapPeer message = objectMapper.readValue(jsonInputStream, CapPeer.class);
    ...
    public void testDecodeMessage1() throws Exception {
        ...
        CapPeer decoded = CephDecoder.decode(byteBuf, true, new BitSet(64), CapPeer.class);
    ...
    private void testEncode(int index) throws Exception {
        CapPeer message = expectedMessages.get(index);
    ...
    private void verifyMessage(CapPeer actual, CapPeer expected) {
        assertEquals(expected.getCapId(), actual.getCapId());
        assertEquals(expected.getIssueSeq(), actual.getIssueSeq());
        assertEquals(expected.getMseq(), actual.getMseq());
        assertEquals(expected.getMds(), actual.getMds());
