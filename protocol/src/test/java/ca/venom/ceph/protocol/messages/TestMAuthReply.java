...
public class TestMAuthReply {
    private static final String MESSAGE_PATH = "sample_messages/messages/MAuthReply/";
    
    private List<MAuthReply> expectedMessages;
    ...
    public void setup() throws Exception {
        ...
        for (int i = 1; i <= 5; i++) {
            ...
            InputStream jsonInputStream = TestMAuthReply.class.getClassLoader().getResourceAsStream(jsonPath);
            InputStream binInputStream = TestMAuthReply.class.getClassLoader().getResourceAsStream(binPath);
            
            ObjectMapper objectMapper = new ObjectMapper();
            MAuthReply message = objectMapper.readValue(jsonInputStream, MAuthReply.class);
        ...
        header.setType((short) MessageType.MSG_AUTH_REPLY.getValueInt());
    }
    
    @Test
    public void testDecodeMessage1() throws Exception {
        ByteBuf byteBuf = Unpooled.wrappedBuffer(binaryMessages.get(0));
        MAuthReply decoded = (MAuthReply) CephDecoder.decodeMessagePayload(byteBuf, true, new BitSet(64), header);
    ...
    private void testEncode(int index) throws Exception {
        MAuthReply message = expectedMessages.get(index);
    ...
    private void verifyMessage(MAuthReply actual, MAuthReply expected) {
        assertEquals(expected.getProtocol(), actual.getProtocol());
        assertEquals(expected.getResult(), actual.getResult());
        assertEquals(expected.getGlobalId(), actual.getGlobalId());
        assertByteArraysEqual(expected.getResultBl(), actual.getResultBl());
        assertStringsEqual(expected.getResultMsg(), actual.getResultMsg());
    }
